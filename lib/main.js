...
const MAX_WORKERS = 5;
const TOTAL_SIMULATION_COUNT = 20;
...
function popupMessageHandler (parameters, sender, responder)
{
	...
	// the parameters.
	workerPool[0].postMessage(command);
	workerPool[1].postMessage(command);
	workerPool[2].postMessage(command);
	workerPool[3].postMessage(command);
	workerPool[4].postMessage(command);
