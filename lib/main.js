...
const TOTAL_SIMULATION_COUNT = 32;
...
function popupMessageHandler (parameters, sender, responder)
{
	...
	var numSimulationsPerThread = Math.floor(TOTAL_SIMULATION_COUNT / WORKER_COUNT);
	...
	var workerMsgHandler = function(messageEvent)
	{
		...
		if (messageEvent.data.message === "finished")
		{
			_Log("Received a 'Finished' event from worker %s", messageEvent.data.id);
			threadsRemaining -= 1;
		}
