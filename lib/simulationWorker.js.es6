...
{	
	//
	// Here I setup three promises using the deferred pattern. Each
	// promise will be resolved when their respective stream is done
	// emitting events.
	//
	...
	attackerRules.setNumDice(parameters.attacker_die_count);
	attackerRules.setNumDieFaces(parameters.attacker_die_type);
	
	var defenderRules = new Ruleset();
	defenderRules.setNumDice(parameters.defender_die_count);
	defenderRules.setNumDieFaces(parameters.defender_die_type);

	// These values represent the army limit that when reached will stop the simulation.
	// By default, Attackers will stop attacking when their armies reach three (since
	// that's when they would begin playing at a disadvantage). For defenders, the
	// simulation doesn't end until all of their armies have been destroyed.
	const minAttackerArmies = 3;
	const minDefenderArmies = 0;

	var attacker = new Player(attackerRules, parameters.attacker_armies, minAttackerArmies);
	var defender = new Player(defenderRules, parameters.defender_armies, minDefenderArmies);
	...
	var avgAttackerArmiesRemaining = simulationOutcomes
		...
		.subscribe(
			armiesRemaining => {
				...
				var num = armiesRemaining * parameters.attacker_armies;
	...
	var averageDefenderArmyLoss = simulationOutcomes
		...
		.subscribe(
			armiesRemaining => {
				var percent = 100 * armiesRemaining;
				var num = armiesRemaining * parameters.defender_armies;
