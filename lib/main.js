...
function popupMessageHandler (message, sender, responder)
{
	...
	var simulationsComplete = new $.Deferred();
	var totalSimulations = 1;
	// ...
	...
	worker.addEventListener('message', function(message)
	{
		...
		{
			console.log("Complete! resolving the simulations");
			simulationsComplete.resolve();
		}
	}, false);
	...
	// otherwise.
	//worker.terminate();
	$.when(simulationsComplete).then(function() {
		console.log("Destroying the worker");
		worker.terminate();
	});
	// ...
